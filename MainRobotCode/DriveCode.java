...
public class Hardware {
	...
	public static final double FLIPPER_RELEASE = .5/5;
	public static final double FLIPPER_CYCLE = 1.0/5;
	...
	public void init() {
		...
		flipper.setPosition(FLIPPER_RELEASE);
...
//			  .setOutputUnits(AprilTagProcessor.OutputUnits.INCHES)  // or METERS if preferred
	...
	/*						Telemetry only functions						  */
	
	public void AutoLaunch(String neededColor) {
		getDetectedColor();
		
		if (normGreen < normBlue && normGreen < normRed) {
			if (neededColor == "Green") {
				Launcher(1);
			}
			else {
				flipper.setPosition(FLIPPER_CYCLE);
				Launcher(1);
			}
		}
	}
